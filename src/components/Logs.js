...
import { filtersFromQuery, queryFromFilters } from './logs/logsQuery';
...
const Logs = () => {
  ...
  const activeMod = (testerMod !== NO_MOD) ? testerMod : cognitoMod;
  const ownMod = (activeMod && activeMod !== NO_MOD) ? activeMod : null;
  ...
  const [filters, setFilters] = useState(() => filtersFromQuery(location.search, defaults));
  ...
  }, [search, location.search, location.pathname, history]);
  ...
  if (error) {
    return (
      <Container fluid>
        <h3 style={{ marginBottom: '1em' }}>Report Logs</h3>
        {notDeployed ? (
