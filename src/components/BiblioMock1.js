...
const BiblioActionToggler = () => {
  ...
  let newUrl = "/BiblioMock1/?action=" + biblioActionTogglerSelected + "&referenceCurie=" + referenceCurie
...
//       let [valueLiveCuriePrefix, valueLiveCurieId] = splitCurie(valueLiveCurie);
      ...
      let valueLiveSource = modAssociationDict['source']; let valueDbSource = ''; let updatedFlagSource = '';
...
const RowDisplayCommentsCorrections = ({fieldIndex, fieldName, referenceJson, referenceJsonLive, referenceJsonDb}) => {
  if (fieldName in referenceJson && referenceJson[fieldName] !== null) {
    ...
    for (const[index, comcorDict] of referenceJson[fieldName].entries()) {
      ...
      const url = '/BiblioMock1/?action=display&referenceCurie=' + valueLiveCurie
...
//              else { obsoleteChecked = ''; } }
      ...
      if (enumDict['mods'].includes(valueLiveCuriePrefix)) {
//         console.log("YES valueLiveCuriePrefix " + valueLiveCuriePrefix);
        rowCrossReferencesElements.push(
          <Form.Group as={Row} key={`${fieldName} ${index}`}>
            <Col className="Col-general form-label col-form-label" sm="2" >{fieldName} </Col>
            <ColEditorSelect key={`colElement ${fieldName} ${index} mod`} fieldType="select" fieldName={fieldName} colSize="2" value={valueLiveCuriePrefix} updatedFlag={updatedFlagCuriePrefix} placeholder="curie" disabled={disabled} fieldKey={`${fieldName} ${index} curie prefix`} enumType="mods" dispatchAction={changeFieldCrossReferencesReferenceJson} />
            <ColEditorSelect key={`colElement ${fieldName} ${index} corpus`} fieldType="select" fieldName={fieldName} colSize={otherColSize} value={valueLiveCorpus} updatedFlag={updatedFlagCorpus} placeholder="corpus" disabled={disabled} fieldKey={`${fieldName} ${index} corpus`} enumType="modAssociationCorpus" dispatchAction={changeFieldCrossReferencesReferenceJson} />
            <ColEditorSelect key={`colElement ${fieldName} ${index} source`} fieldType="select" fieldName={fieldName} colSize={otherColSizeB} value={valueLiveSource} updatedFlag={updatedFlagSource} placeholder="source" disabled={disabled} fieldKey={`${fieldName} ${index} source`} enumType="modAssociationSource" dispatchAction={changeFieldCrossReferencesReferenceJson} />
            {revertElement}
          </Form.Group>); } } }
...
//     if (paramReferenceCurie !== null) { dispatch(setLoadingQuery(true)); }
  ...
  if (referenceCurie !== '' && (getReferenceCurieFlag === true)) {
    console.log('biblio DISPATCH biblioMock1QueryReferenceCurie ' + referenceCurie);
    dispatch(biblioMock1QueryReferenceCurie(referenceCurie));
  }
