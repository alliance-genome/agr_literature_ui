...
const completeMergeReferences = async (dispatch, accessToken, referenceMeta1, referenceMeta2) => {
  try {
    dispatch(setMergeCompleting(1));
    
    const mergeAndPoll = async () => {
      const subPath = `reference/merge/${referenceMeta2.curie}/${referenceMeta1.curie}`;
      const response = await axios.post(
        `${process.env.REACT_APP_API_URL}/${subPath}`,
        {},
        { headers: { Authorization: `Bearer ${accessToken}` } }
      );

      // If merge needs processing, start polling
      if (response.data.messages.some(msg => msg.includes("is currently being processed"))) {
        let attempts = 0;
        const maxAttempts = 30; // 30 attempts * 10s = 5min timeout
        
        while (attempts < maxAttempts) {
          await new Promise(resolve => setTimeout(resolve, 10000));
          const statusCheck = await axios.get(
            `${process.env.REACT_APP_API_URL}/reference/${referenceMeta1.curie}`,
            { headers: { Authorization: `Bearer ${accessToken}` } }
          );
          ...
          dispatch({
            ...
            payload: statusCheck.data.messages
          });

          if (!statusCheck.data.messages.some(msg => msg.includes("is currently being processed"))) {
            return true; // Merge completed
          }
          attempts++;
        }
        throw new Error("Merge processing timeout");
      }
      return true; // Immediate success
    };

    await mergeAndPoll();
    dispatch(setMergeCompleting(0));
    dispatch({ type: 'MERGE_COMPLETE_SUCCESS' });

  } catch (error) {
    dispatch(setMergeCompleting(0));
    dispatch({
      type: 'MERGE_COMPLETE_FAILURE',
      payload: error.response?.data?.messages || [error.message]
    });
  }
};
...
const MergeCompletedMergeModal = () => {
  ...
  useEffect(() => {
    ...
    paperUnlockedRef.current = paperUnlocked;
  }, [secondsLeft, paperUnlocked]);
...
useEffect(() => {
  if (completionMergeHappened && !updateFailure) {
    // Only show success if no processing messages remain
    const stillProcessing = updateMessages.some(msg => 
      msg.includes("is currently being processed")
    );
    
    if (stillProcessing) {
      // Reset completion state to continue polling
      dispatch(setMergeCompleting(1));
      completeMergeReferences(dispatch, accessToken, referenceMeta1, referenceMeta2);
    }
  }
}, [completionMergeHappened, updateMessages]);
  ...
  useEffect(() => {
    let pollingInterval;
    if (isProcessing && !paperUnlocked && secondsLeftRef.current > 0) {
	  ...
	  console.log("HELLO secondsLeftRef=", secondsLeftRef.current);
	  console.log("HELLO updateMessages=", updateMessages); 
      }, 10000);
    }
    return () => clearInterval(pollingInterval);
  }, [isProcessing, paperUnlocked, dispatch, accessToken, referenceMeta1, referenceMeta2]);

  // when updateMessages no longer contain the warning text, mark the paper as unlocked.
  useEffect(() => {
      if (isProcessing && updateMessages) {
	  const hasWaitingMessage = updateMessages.some(msg => msg.includes(processingText));
	  if (!hasWaitingMessage && !paperUnlocked) {
 	      setPaperUnlocked(true);
	      setSecondsLeft(0);
	  }
      }
  }, [updateMessages, isProcessing, paperUnlocked]);
...
const MergeDataTransferredModal = () => {
  ...
  const modalBody = updateFailure ? 
        <Modal.Body>Data has been merged from {referenceMeta2.curie} into {referenceMeta1.curie} but something went wrong, updateMessage={updateMessages}, <a href="/merge" target="_blank" rel="noreferrer noopener">try again in a new browser tab</a>.</Modal.Body> :
...
//   const completionMergeHappened = useSelector(state => state.merge.completionMergeHappened);
  ...
  const isProcessing = updateMessages?.some(msg => 
    msg.includes("is currently being processed")
  );
  ...
  return (<>
     <Button
	  variant='primary'
	  onClick={() => completeMergeReferences(dispatch, accessToken, referenceMeta1, referenceMeta2)}
	  disabled={mergeCompletingCount > 0 || isProcessing}
     >
