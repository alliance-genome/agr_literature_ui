// src/components/settings/LaboratoryEditorLayoutModal.js
//
// Graphical preference modal for arranging the Laboratory Editor's sections in a
// 2D grid. Adapted from PersonEditorLayoutModal; the only differences are the
// section definitions, the persistence namespace (`laboratory_editor_layout`),
// and element ids. Per-user settings are stored via the shared /person_setting
// API (keyed by the logged-in curator), reused here through usePersonSettings.
...
} from '../laboratory/laboratoryEditorSections';
...
const LaboratoryEditorLayoutModal = ({
  onApplyPrefs,
  current,
  onToggleSection,
  onToggleTimestamps,
  ...
  useEffect(() => {
    ...
    load()
      ...
      .catch((err) => {
        ...
        console.error('Failed to load Laboratory editor layout preferences:', msg);
      });
...
export default LaboratoryEditorLayoutModal;
