...
// --- Facet renaming & grouping definitions ---
...
const WorkflowTagsFacet = ({ aggregations, onSelectTerm }) => {
  console.log("WorkflowTagsFacet aggregations:", aggregations);
  // Instead of expecting a .buckets property, use the object directly.
  const subcatsAgg = aggregations && aggregations.workflow_tags_subcategories;
  if (!subcatsAgg || Object.keys(subcatsAgg).length === 0) {
    return (
      <div style={{ paddingLeft: "1em", fontStyle: "italic" }}>
        No Workflow Tags found
      </div>
    );
  }

  // Convert subcategory keys into user-friendly labels.
  ...
  return (
    <div className="workflow-tags-facet" style={{ paddingLeft: "1em" }}>
      {Object.keys(subcatsAgg).map((subcatKey) => {
        const bucket = subcatsAgg[subcatKey];
        const termAgg = bucket.terms; 
        const termBuckets =
          termAgg && termAgg.buckets
            ? termAgg.buckets.filter((term) => term.doc_count > 0)
            : [];
        if (termBuckets.length === 0) return null;
        return (
          <div key={subcatKey} className="workflow-subcategory" style={{ marginBottom: "0.5em" }}>
            <div className="subcategory-header">
              <span className="subcategory-title" style={{ fontWeight: "bold" }}>
                {formatSubcategoryLabel(subcatKey)}
              </span>{" "}
              <span className="subcategory-count">({bucket.doc_count})</span>
            </div>
            <ul className="subcategory-terms list-unstyled">
              {termBuckets.map((term) => (
                <li key={term.key} className="term-item">
                  ...
                  >
                    {term.name ? term.name : term.key} <Badge variant="secondary">{term.doc_count}</Badge>
                  </Button>
                </li>
              ))}
            </ul>
  ...
  return (
    <div key={facetName} style={{ textAlign: "left", paddingLeft: "2em", paddingBottom: "0.5em" }}>
      ...
      <ButtonGroup aria-label="DateSetter" size="sm" style={{ display: "block" }}>
        <Button variant="secondary" style={{ borderBottomLeftRadius: 0 }} onClick={() => handleFixedTimeClick("Day")}>
          Day
        </Button>
        <Button variant="secondary" onClick={() => handleFixedTimeClick("Week")}>
          Week
        </Button>
        <Button variant="secondary" onClick={() => handleFixedTimeClick("Month")}>
          Month
        </Button>
        <Button variant="secondary" style={{ borderBottomRightRadius: 0 }} onClick={() => handleFixedTimeClick("Year")}>
          Year
        </Button>
      </ButtonGroup>
...
const Facet = ({ facetsToInclude, renameFacets }) => {
  ...
  const NegatedFacetCheckbox = ({ facet, value }) => {
    ...
    return (
      <span>
        <FontAwesomeIcon
          icon={faCheckSquare}
          style={includedChecked ? { color: "#28a745" } : { color: "#808080" }}
          onClick={handleClickInclude}
        />
        &nbsp;
        <FontAwesomeIcon
          icon={faMinusSquare}
          style={excludedChecked ? { color: "#dc3545" } : { color: "#808080" }}
  ...
  return (
    <div className="facet-container">
      {Object.entries(searchFacets).length > 0 &&
                      ...
                      <Row className="align-items-center facet-item">
                        ...
                        <Col xs={3} sm={3}>
                          <Badge variant="secondary">
                            {['topics', 'confidence_levels', 'source_methods', 'source_evidence_assertions'].includes(key) && bucket.docs_count !== undefined
  ...
  return (
    <InputGroup size="sm" className="mb-3" style={{ width: "85%" }}>
      <Form.Control
        ...
        id="authorFilter"
        name="authorFilter"
        placeholder="Filter Authors (case sensitive)"
        value={authorFilter}
      ...
      />
      <Button inline="true" style={{ width: "4em" }} size="sm" onClick={() => dispatch(filterFacets())}>
        Filter
      </Button>
      <Button
        variant="danger"
      ...
      {searchFacetsLimits[facetLabel] > INITIAL_FACETS_LIMIT ? (
        <span>
          ...
          <button
            ...
            onClick={() => {
              let newSearchFacetsLimits = _.cloneDeep(searchFacetsLimits);
              newSearchFacetsLimits[facetLabel] = INITIAL_FACETS_LIMIT;
              dispatch(setSearchFacetsLimits(newSearchFacetsLimits));
              dispatch(filterFacets());
            }}
          >
      ...
      {facetValue.buckets && facetValue.buckets.length >= searchFacetsLimits[facetLabel] ? (
        <span>
          ...
          <button
            ...
            onClick={() => {
              let newSearchFacetsLimits = _.cloneDeep(searchFacetsLimits);
              newSearchFacetsLimits[facetLabel] = 1000;
              dispatch(setSearchFacetsLimits(newSearchFacetsLimits));
              dispatch(filterFacets());
            }}
          >
...
const Facets = () => {
  ...
  useEffect(() => {
    if (Object.keys(searchFacets).length === 0 && searchResults.length === 0) {
      ...
      if (
        readyToFacetSearch === true &&
        (searchQuery !== "" ||
          searchResults.length > 0 ||
          Object.keys(searchFacetsValues).length > 0 ||
          Object.keys(searchExcludedFacetsValues).length > 0)
      ) {
        dispatch(setSearchResultsPage(1));
        dispatch(setAuthorFilter(""));
        dispatch(searchReferences());
    ...
    <>
      ...
      {Object.entries(FACETS_CATEGORIES_WITH_FACETS).map(([facetCategory, facetsInCategory]) => (
        <div key={facetCategory} style={{ textAlign: "left" }}>
          <Button variant="light" size="lg" onClick={() => toggleFacetGroup(facetCategory)}>
